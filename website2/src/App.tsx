...
import DeliverabilityZigZag from "./components/DeliverabilityZigZag"
...
function HeroBackground() {
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none flex items-center justify-center">
      ...
      {/* Abstract Geometry Replacements */}
      {/* Abstract Data Block 1 */}
      <motion.div
        animate={{
          y: [-20, 20, -20],
        ...
        className="absolute left-[15%] top-[25%] w-24 h-24 md:w-32 md:h-32 bg-primary/5 border border-primary/20 backdrop-blur-md rounded-xl"
      />
      
      {/* Abstract Data Block 2 */}
      <motion.div
        animate={{
          y: [20, -20, 20],
        ...
        className="absolute right-[15%] bottom-[25%] w-24 h-24 md:w-40 md:h-40 bg-foreground/5 border border-foreground/10 backdrop-blur-md rounded-xl"
      />
      ...
      <motion.div
        ...
        className="absolute right-[25%] top-[20%] w-32 h-32 md:w-48 md:h-48 bg-orange-500/5 border border-orange-500/20 rounded-full backdrop-blur-sm"
      />
      ...
      <motion.div
        ...
        className="absolute left-[20%] bottom-[20%] w-40 h-40 md:w-64 md:h-64 bg-primary/5 border border-primary/20 rounded-full backdrop-blur-sm"
      />
    </div>
  )
}
...
function FAQItem({ question, answer }: { question: string, answer: string }) {
  ...
  return (
    <div className="border border-border/50 bg-card/95 rounded-lg overflow-hidden transition-colors hover:bg-card">
      <button 
        onClick={() => setIsOpen(!isOpen)} 
        className="w-full p-6 flex justify-between items-center text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-primary transition-all duration-300 active:scale-[0.99]"
      >
    ...
    <section className="w-full max-w-7xl mt-32 grid md:grid-cols-2 gap-16 items-center px-4 overflow-visible">
      ...
      <div className="relative w-full h-[400px] md:h-[500px] flex items-center justify-center ml-0 md:-ml-8">
        ...
        <motion.div 
          ...
          className="absolute z-0 w-72 md:w-80 h-80 md:h-96 bg-card/95 border border-border rounded-xl shadow-2xl p-4 flex flex-col grayscale opacity-60 overflow-hidden"
        >
        ...
        <motion.div 
          ...
          className="absolute z-10 w-72 md:w-80 h-80 md:h-96 bg-card/95 border border-primary/30 rounded-xl shadow-[0_0_50px_-12px_rgba(0,100,255,0.3)] p-6 flex flex-col"
        >
...
export default function App() {
  return (
    <ReactLenis root options={{ duration: 1.4 }}>
      <div className="relative min-h-screen w-full bg-background overflow-clip">
        
        {/* CSS-based Hero Background */}
        <div className="absolute inset-0 z-0 pointer-events-none">
          <HeroBackground />
        </div>

        <header className="fixed top-0 left-0 right-0 z-50 bg-background/95 border-b border-border/50 transition-all duration-300 py-4">
          <div className="container mx-auto px-6 flex justify-between items-center">
            <div className="text-xl font-display font-black tracking-widest uppercase text-foreground">
              <img src="/logo.png" alt="Email Pilots logo" className="h-8 w-auto object-contain" />
            </div>
            <nav className="hidden md:flex items-center gap-8 text-sm font-semibold tracking-widest uppercase text-muted-foreground">
              <a href="#personas" className="hover:text-primary transition-colors duration-300">Personas</a>
              <a href="#features" className="hover:text-primary transition-colors duration-300">Features</a>
              <a href="#pricing" className="hover:text-secondary transition-colors duration-300">Pricing</a>
              <a href="#faq" className="hover:text-primary transition-colors duration-300">FAQ</a>
            </nav>
            <Button size="sm" className="hidden md:inline-flex bg-primary text-primary-foreground hover:bg-primary/90 font-semibold tracking-wide rounded-md transition-all duration-300 active:scale-95">
              Start Free Trial
            </Button>
          </div>
        </header>
        ...
        <main className="relative z-10 container mx-auto px-4 py-24 flex flex-col items-center justify-center gap-16 text-foreground">
          ...
          >
            <h1 className="text-5xl md:text-7xl lg:text-8xl font-display font-black tracking-normal mb-8 leading-tight text-foreground [-webkit-text-stroke:3px_hsl(var(--foreground))] md:[-webkit-text-stroke:4px_hsl(var(--foreground))] [paint-order:stroke_fill] drop-shadow-2xl max-w-5xl mx-auto">
              Personalized Email Outreach <br className="hidden lg:block" />
              <span className="text-secondary [-webkit-text-stroke:3px_hsl(var(--secondary))] md:[-webkit-text-stroke:4px_hsl(var(--secondary))] [paint-order:stroke_fill]">from Your Own Inbox.</span>
            </h1>
            <div className="bg-background/95 px-8 py-6 rounded-3xl border border-border shadow-2xl mb-12 max-w-3xl mx-auto">
            ...
            <div className="flex gap-4 justify-center flex-wrap">
              <Button size="lg" className="bg-primary text-primary-foreground hover:bg-primary/90 transition-all duration-300 active:scale-95">
                Start Sending Now
              </Button>
              <Button size="lg" variant="outline" className="border-primary text-primary hover:bg-primary/10 transition-all duration-300 active:scale-95">
                Watch Demo
              </Button>
            </div>
            <motion.div 
              ...
              className="mt-6 flex items-center justify-center gap-2 text-sm font-semibold text-muted-foreground"
            >
              <span className="w-2 h-2 rounded-full bg-green-500 animate-pulse"></span>
              Start sending for less than the price of a coffee.
            </motion.div>
          </motion.section>
          ...
          <section id="personas" className="w-full max-w-7xl mt-48 px-4 flex flex-col gap-32 mb-32">
            ...
            >
              <h3 className="text-[15vw] md:text-[clamp(4rem,8vw,10rem)] leading-[0.9] font-display font-black tracking-tighter text-foreground/15 group-hover:text-primary transition-all duration-1000 group-hover:duration-300 uppercase select-none relative z-0">
                Job <br /> Seekers.
              </h3>
              <motion.div 
                initial={{ opacity: 0, x: 50 }}
                ...
                className="max-w-xl mt-8 p-8 md:p-10 md:self-end bg-background/95 border border-border group-hover:border-primary/30 group-hover:shadow-[0_20px_60px_-15px_rgba(0,85,255,0.3)] transition-all duration-1000 group-hover:duration-300 relative z-10"
              >
            ...
            >
              <h3 className="text-[15vw] md:text-[clamp(4rem,8vw,10rem)] leading-[0.9] font-display font-black tracking-tighter text-foreground/15 group-hover:text-secondary transition-all duration-1000 group-hover:duration-300 uppercase text-right select-none relative z-0">
                Recruiters.
              </h3>
              <motion.div 
                initial={{ opacity: 0, x: -50 }}
                ...
                viewport={{ once: true, amount: 0.25 }}
                transition={{ duration: 1.2, delay: 0.15, ease: "easeOut" }}
                className="max-w-xl mt-8 p-8 md:p-10 md:self-start bg-background/95 border border-border group-hover:border-secondary/30 group-hover:shadow-[0_20px_60px_-15px_rgba(255,102,0,0.3)] transition-all duration-1000 group-hover:duration-300 text-left relative z-10"
              >
          ...
          <section id="pricing" className="w-full max-w-5xl mt-24 mb-48 px-4 relative z-10 flex flex-col items-center">
            ...
            <div className="flex flex-col md:flex-row gap-6 max-w-3xl w-full justify-center items-end">
              ...
              <div className="w-full md:w-[320px] bg-white rounded-[2rem] p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-black/5 hover:shadow-[0_20px_40px_rgb(0,85,255,0.1)] hover:-translate-y-1 transition-all duration-300">
                ...
                <button className="w-full py-4 bg-primary text-white font-bold rounded-2xl hover:bg-primary/90 transition-all duration-300 active:scale-95 shadow-sm">
                  Get Started
                </button>
              </div>
              ...
              <div className="w-full md:w-[320px] bg-gradient-to-b from-white to-secondary/5 rounded-[2rem] p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-secondary/20 relative mt-8 md:mt-0 opacity-95">
                ...
                <button className="w-full py-4 bg-secondary/10 text-secondary font-bold rounded-2xl cursor-not-allowed transition-all duration-300" disabled>
                  Join Waitlist
                </button>
              </div>
          ...
          <div className="relative w-full rounded-xl bg-gradient-to-br from-primary to-[#002288] overflow-hidden flex flex-col items-center text-center px-6 py-24 shadow-2xl border border-primary/20">
            ...
            <div className="relative z-10 flex flex-col items-center">
              ...
              <div className="flex flex-col sm:flex-row gap-6 w-full sm:w-auto justify-center items-center">
                <button className="group flex items-center justify-center gap-3 px-8 py-4 bg-white text-primary font-semibold text-base rounded-lg hover:bg-white/90 transition-all duration-300 shadow-xl hover:shadow-2xl active:scale-95 w-full sm:w-auto">
                  Start sending free
                ...
                <button className="px-8 py-4 bg-transparent border border-white/30 text-white font-semibold text-base rounded-lg hover:bg-white/10 hover:border-white transition-all duration-300 active:scale-95 w-full sm:w-auto backdrop-blur-sm">
                  See how it works
                </button>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-12 lg:gap-8 mb-16">
              <div className="lg:col-span-2">
                <div className="text-2xl font-display font-black tracking-widest uppercase text-foreground mb-6">
                  <img src="/logo.png" alt="Email Pilots logo" className="h-10 w-auto object-contain" />
                </div>
              ...
              <div>
                ...
                <ul className="space-y-4 text-muted-foreground">
                  <li><a href="#job-seekers" className="hover:text-primary transition-colors duration-300">Job seekers</a></li>
                  <li><a href="#recruiters" className="hover:text-primary transition-colors duration-300">Recruiters</a></li>
                  <li><a href="#freelancers" className="hover:text-primary transition-colors duration-300">Freelancers</a></li>
                  <li><a href="#community-builders" className="hover:text-primary transition-colors duration-300">Community builders</a></li>
                </ul>
              </div>
              ...
              <div>
                ...
                <ul className="space-y-4 text-muted-foreground">
                  ...
                  <li><a href="mailto:support@emailpilots.com" className="hover:text-primary transition-colors duration-300">Contact</a></li>
                </ul>
              </div>
            </div>
